...
void DisplayFunction() {
	...
	//#if 0
	glClearColor(1,1,0,0); // Blue background (Red = 0, Green = 0, Blue = 1)
glClear(GL_COLOR_BUFFER_BIT); // Update the color buffer

	//glClearColor ( 1) Red Component, ( 0) Green Component,
	...
	for(int j=0;j<15;j++)
	{	
	DrawAlphabet((alphabets)a[i][j],x+10, height-160-h, awidth, aheight);
		x=x+60;
	}
	h=h+60;
	}
	//DrawAlphabet((alphabets)shooterA,width/2, 10, awidth, aheight);
	DrawAlphabet((alphabets)shooterA,l,b, awidth, aheight);
	...
	DrawString(width / 2 - 30, height - 25, width, height,
		"Time Left:" + Num2Str(t/100) + " secs", colors[RED]);
		
		
		
                DrawString(width/22,height-50,width,height,"MARYAM ABID",colors[BLUE_VIOLET]);
                  DrawString(width/22,height-80,width,height,"240114",colors[BLUE_VIOLET]);
